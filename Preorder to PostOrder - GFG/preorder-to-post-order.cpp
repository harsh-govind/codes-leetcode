...
public:
    //Function that constructs BST from its preorder traversal.
     Node* solve(int preorder[], int &i, int mn, int mx, int n)
    {
        if(i>=n) return nullptr;
        
        if(preorder[i]<mn or preorder[i]>mx) return nullptr;
        
        Node *node=newNode(preorder[i++]);
        
        node->left=solve(preorder, i, mn, node->data, n);
        node->right=solve(preorder, i, node->data, mx, n);
